...
struct TickerView: View {
    ...
    private enum LogLevel: Int { case info = 1 }

    var body: some View {
        let barBG = FeedsTheme.background.opacity(tickerOpacity)
        let fixedBG = FeedsTheme.background

        return ZStack(alignment: .leading) {
            barBG.ignoresSafeArea()
        ...
        .onAppear {
            dbg("TICKERVIEW appeared", level: .info)
            dbg("TICKERVIEW items.count = \(feedManager.items.count)", level: .info)
    ...
    private func dbg(_ msg: String, level: LogLevel) {
        print("TICKER: \(msg)")
    }
}
...
struct TickerIconView: View {
    ...
    var body: some View {
        // Wrap everything in a nice rounded square background
        ZStack {
            RoundedRectangle(cornerRadius: 6)
                .fill(FeedsTheme.inputBackground)
                .overlay(RoundedRectangle(cornerRadius: 6).stroke(FeedsTheme.divider.opacity(0.7), lineWidth: 1))

            if shouldSkipNetworkFavicon(item) {
                fallbackBadge(symbol: fallbackSymbol(for: item))
            } else {
                // Use FaviconStore directly to get the image or fallback logic
                if let domain = domainFor(item),
                   let img = faviconStore.image(for: domain, size: 64) {
                    
                    ZStack {
                        // Light white backing behind transparent icons
                        Circle().fill(Color.white.opacity(0.12)).frame(width: iconSize + 6, height: iconSize + 6)
                    ...
                    // While loading or if failed (and not in cache yet), show fallback
                    fallbackBadge(symbol: fallbackSymbol(for: item))
                        .onAppear {
    ...
    private var boxSize: CGFloat {
        size == 1 ? 28 : (size == 4 ? 54 : 40)
    }

    private var iconSize: CGFloat {
        size == 1 ? 16 : (size == 4 ? 32 : 24)
    }
...
struct FixedBrandBlock: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ZStack(alignment: .leading) {
                background.frame(width: blockWidth(size))
    ...
    private func blockWidth(_ size: Int) -> CGFloat { size == 1 ? 210 : (size == 4 ? 360 : 270) }
...
struct NewsSentimentOrb: View {
    ...
    var body: some View {
        ...
        .onAppear { if feedManager.isComputingSentiment { pulse = true } }
        .onChange(of: feedManager.isComputingSentiment) { _, computing in pulse = computing }
    }

    private var orbSize: CGFloat { size == 1 ? 12 : (size == 4 ? 22 : 16) }
    private var ringWidth: CGFloat { size == 1 ? 1.2 : (size == 4 ? 2.0 : 1.6) }
    private var summaryFontSize: CGFloat { size == 4 ? 12 : 9 }

    private var orbCoreColor: Color {
        guard let level = feedManager.newsSentiment?.level else { return Color.white.opacity(0.35) }
        switch level {
    ...
    private var helpText: String {
        if feedManager.isComputingSentiment { return "Computing today's news sentiment…" }
        if let s = feedManager.newsSentiment?.threeWordSummary { return "News mood: \(s). Click to refresh." }
